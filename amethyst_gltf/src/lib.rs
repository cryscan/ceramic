...
pub type GltfSceneLoaderSystemDesc<T> = PrefabLoaderSystemDesc<GltfPrefab<T>>;

/// Load `GltfSceneAsset`s
pub type GltfSceneLoaderSystem<T> = PrefabLoaderSystem<GltfPrefab<T>>;

/// Gltf scene asset as returned by the `GltfSceneFormat`
pub type GltfSceneAsset<T> = Prefab<GltfPrefab<T>>;

/// `PrefabData` for loading Gltf files.
#[derive(Debug, Default)]
pub struct GltfPrefab<T> {
...
impl<'a, T> PrefabData<'a> for GltfPrefab<T>
    where T: PrefabData<'a> {
    type SystemData = (
    ...
    fn add_to_entity(
        ...
        if let Some(custom) = &self.extras {
            custom.add_to_entity(entity, extras, entities, children)?;
        }
    ...
    fn load_sub_assets(
        ...
        let (_, _, _, materials, animatables, _, _, _, _, _, meshes_storage, loader, mat_set) =
            system_data;
